...
$$
\begin{aligned}
m &= \max(x_1, \ldots, x_N)            &&\text{— 第 1 次遍历: 找最大值} \\
y_i &= \exp(x_i - m)                    &&\text{— 第 2 次遍历: 计算 exp} \\
s &= \sum_i y_i                         &&\text{— 累加} \\
\text{softmax}(x_i) &= y_i / s          &&\text{— 第 3 次遍历: 归一化}
\end{aligned}
$$
...
\begin{aligned}
&\text{初始化:} \\
&\quad m = -\infty, \quad s = 0 \\
\\
&\text{对于每个新元素 } x_i: \\
&\quad m_{\text{new}} = \max(m, x_i) \quad \text{— 更新 running max} \\
&\quad s = s \cdot \exp(m - m_{\text{new}}) \quad \text{← 关键！旧的 } s \text{ 是基于旧 max 算的，需要缩放到新 max 的基准} \\
&\quad s = s + \exp(x_i - m_{\text{new}}) \quad \text{← 新元素基于新 max} \\
&\quad m = m_{\text{new}} \quad \text{— 更新 max} \\
\\
&\text{所有元素处理完后:} \\
&\quad \text{softmax}(x_i) = \frac{\exp(x_i - m)}{s}
\end{aligned}
$$
...
\\
&x_0 = 2: \\
&\quad m_{\text{new}} = \max(-\infty, 2) = 2 \\
&\quad s = 0 \cdot \exp(-\infty) + \exp(2-2) = 1.0 \quad (\text{旧 } s=0\text{，直接加新值}) \\
&\quad m = 2 \\
\\
&x_1 = 1: \\
&\quad m_{\text{new}} = \max(2, 1) = 2 \quad (\text{max 没变}) \\
&\quad s = 1.0 \cdot \exp(2-2) + \exp(1-2) \\
&\quad = 1.0 + \exp(-1) = 1.0 + 0.368 = 1.368 \\
&\quad m = 2 \\
\\
&x_2 = 4: \\
&\quad m_{\text{new}} = \max(2, 4) = 4 \quad (\text{max 变了！}) \\
&\quad s = 1.368 \cdot \exp(2-4) + \exp(4-4) \\
&\quad = 1.368 \cdot 0.135 + 1.0 = 0.185 + 1.0 = 1.185 \\
&\quad m = 4 \\
\\
&x_3 = 3: \\
&\quad m_{\text{new}} = \max(4, 3) = 4 \quad (\text{max 没变}) \\
&\quad s = 1.185 \cdot \exp(4-4) + \exp(3-4) \\
&\quad = 1.185 + \exp(-1) = 1.185 + 0.368 = 1.553 \\
&\quad m = 4
\end{aligned}
$$
...
$$
\begin{aligned}
&\text{For each Q block } (Q_i, \text{大小为 } B_r \times d): \\
&\quad O_i = 0,\; l_i = 0,\; m_i = -\infty \quad \text{← 初始化 running 状态} \\
&\quad \text{For each KV block } (K_j, V_j, \text{大小为 } B_c \times d): \\
&\quad\quad S_{ij} = Q_i K_j^T \quad \text{← attention scores for this block} \\
&\quad\quad m_{ij} = \text{rowmax}(S_{ij}) \quad \text{← local max} \\
&\quad\quad m_{\text{new}} = \max(m_i, m_{ij}) \quad \text{← running max} \\
&\quad\quad P_{ij} = \exp(S_{ij} - m_{\text{new}}) \quad \text{← stable exp (用新 max)} \\
&\quad\quad l_{\text{new}} = \exp(m_i - m_{\text{new}}) \cdot l_i + \text{rowsum}(P_{ij}) \quad \text{← rescale + update sum} \\
&\quad\quad O_i = \operatorname{diag}(\exp(m_i - m_{\text{new}})) \cdot O_i + P_{ij} V_j \quad \text{← rescale + accumulate} \\
&\quad\quad m_i = m_{\text{new}},\; l_i = l_{\text{new}} \quad \text{← update running state} \\
&\quad O_i = \operatorname{diag}(1/l_i) \cdot O_i \quad \text{← final normalize} \\
&\quad \text{Write } O_i \text{ to HBM}
\end{aligned}
$$
...
\begin{aligned}
&\text{v1 (KV 外循环):} \\
&\quad \text{For each } K_j, V_j: \\
&\quad\quad \text{Load } K_j, V_j \text{ (on chip)} \\
&\quad\quad \text{For each } Q_i: \\
&\quad\quad\quad \text{Compute } S_{ij}, \text{ update } O_i \\
\\
&\text{特点: KV block 被所有 Q block 复用 → 更好的 KV 数据复用} \\
&\text{问题: 每个 Q block 需要维护自己的 running state } (m, l, O) \\
&\qquad\quad \text{→ 多个 Q block 之间没有交流} \\
&\qquad\quad \text{→ warp divergent} \\
\\
&\text{v2 (Q 外循环):} \\
&\quad \text{For each } Q_i: \\
&\quad\quad \text{Load } Q_i \text{ (on chip)} \\
&\quad\quad m_i = -\infty,\; l_i = 0,\; O_i = 0 \\
&\quad\quad \text{For each } K_j, V_j: \\
&\quad\quad\quad \text{Compute } S_{ij}, \text{ update } m_i, l_i, O_i \\
&\quad\quad O_i = O_i / l_i \quad \text{← 最终归一化只在 Q 循环末尾做一次} \\
&\quad\quad \text{Write } O_i \\
\\
&\text{改进:} \\
&\quad 1. \text{ 最终归一化从内循环移到外循环 — 减少 non-matmul FLOPs} \\
&\quad 2. \text{ Q 外循环 — 同一 warp 内所有线程处理同一个 Q tile，更少的 divergence} \\
&\quad 3. \text{ 支持 causal masking（只需要遍历 } K_j \leq Q_i \text{ 的块）}
\end{aligned}
$$
...
\begin{aligned}
&\text{for } K_j: \\
&\quad O_i = \alpha \cdot O_i + P_{ij} V_j \quad \text{← 每次都要 rescale}
\end{aligned}
$$
...
$$
\begin{aligned}
&\text{// 内循环不 rescale } O_i \text{，只更新 sum} \\
&\text{for } K_j: \\
&\quad P_{ij} = \exp(S_{ij}) \\
&\quad O_i = O_i + P_{ij} V_j \quad \text{← 不做 rescale！} \\
&\quad l_i = l_i + \text{sum}(P_{ij}) \quad \text{← 只累加 sum} \\
\\
&\text{// 外循环统一做 rescale + normalize} \\
&\quad O_i = O_i / l_i
\end{aligned}
$$
