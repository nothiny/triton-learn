...
GPU 的硬件线程切换开销**几乎为零**（所有 warp 的寄存器都在 on-chip SRAM 中），所以它可以：
1. Warp 0 等待内存 → 立即切换到 Warp 1
2. Warp 1 等待内存 → 立即切换到 Warp 2
3. ...

### 4.2 Occupancy 公式

```
Occupancy = 当前 SM 上活跃的 warp 数 / SM 最大 warp 数
```
...
示例计算:
  每线程用 255 个寄存器 × 32 threads/warp = 8160 寄存器/warp
  → 65536 / 8160 = 8 个 warp（最多）
  → occupancy = 8/64 = 12.5%（很低！）

  每线程用 64 个寄存器 × 32 threads/warp = 2048 寄存器/warp
  → 65536 / 2048 = 32 个 warp
  → occupancy = 32/64 = 50%（不错）
```
...
```
普通 CUDA Core（FP32）:
  一次做 1 个乘法 + 1 个加法 = 1 FMA
  需要 2 cycles
  
Tensor Core（FP16）:
  一次做 16×8×16 的矩阵乘加 = 2048 个 FMA
  只需要 1 cycles
